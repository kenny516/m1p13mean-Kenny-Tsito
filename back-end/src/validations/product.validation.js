...
export const updateProductSchema = Joi.object({
	...
	moderation: Joi.object({
		status: Joi.string().valid("DRAFT", "PENDING", "ARCHIVED").optional().messages({
...
export const listProductsQuerySchema = Joi.object({
	...
	tags: Joi.alternatives().try(Joi.string(), Joi.array().items(Joi.string())).optional().messages({
		"alternatives.match": "Les tags doivent être une chaîne ou un tableau de chaînes",
	}),
	...
	shopId: Joi.string()
		...
		.optional()
		.messages({
			"string.pattern.base": "L'ID de la boutique doit être un ObjectId MongoDB valide",
		}),
...
export const moderateProductSchema = Joi.object({
	moderation: Joi.object({
		...
		.required()
		.messages({
			"any.required": "Les informations de modération sont requises",
		}),
}).strict();
...
export const reserveStockSchema = Joi.object({
	...
	action: Joi.string().valid("reserve", "release", "commit").required().messages({
