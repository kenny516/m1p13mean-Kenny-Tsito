...
const _getStockSnapshot = async (product, useCache) => {
	...
	if (useCache && cache && typeof cache.total === "number" && typeof cache.reserved === "number") {
		return {
			total: cache.total,
...
const _checkAvailability = async ({ product, movementType, quantity, useCache }) => {
...
const _syncHeaderData = async (moveId, session) => {
	...
	const totalAmount = lines.reduce((sum, line) => sum + (line.totalAmount || 0), 0);
...
const _resolveProductAndShop = async ({ productId, shopId }, session, providedProduct) => {
	...
	if (product.status !== "ACTIVE") {
		throw new ApiError(400, "PRODUCT_NOT_ACTIVE", "Le produit doit être au statut ACTIVE");
	}
	if (product.shopId.toString() !== resolvedShopId) {
		throw new ApiError(400, "INVALID_SHOP", "Le produit n'appartient pas a cette boutique");
	}
...
const _createHeader = async (payload, performedBy, session) => {
	...
	if (payload.movementType === "RESERVATION" || payload.movementType === "RESERVATION_CANCEL") {
	...
	if (payload.movementType === "ADJUSTMENT_PLUS" || payload.movementType === "ADJUSTMENT_MINUS") {
...
const _createLine = async (header, item, options) => {
	...
	const useCache = options.cache !== undefined ? options.cache : options.useCache !== false;
	const providedProduct = item.product || options.product || null;

	const { product, resolvedProductId, resolvedShopId } = await _resolveProductAndShop(
	...
	_applyMovementToCache(product, header.movementType, item.quantity, header.direction);
...
export const createCartMovementLine = async (payload, performedBy, options = {}) => {
	...
	try {
		...
		const { cartId, productId, quantity, unitPrice = 0, shopId, date } = payload;

		let line = await StockMovementLine.findOne({
			movementType,
			cartId,
			productId,
		}).session(session);
		...
		if (line) {
			...
			let header = await StockMovement.findOne({
				movementType,
				cartId,
			})
			...
			if (!header) {
				header = await _createHeader({ movementType, cartId, date }, performedBy, session);
			}
			...
			[line] = await StockMovementLine.create([lineDoc], { session });
		}

		_applyMovementToCache(product, movementType, quantity, _getDirection(movementType));
		await product.save({ session });

		await _syncHeaderData(line.moveId, session);
...
export const createStockMovement = async (payload, performedBy, options = {}) => {
	const providedSession = options.session || null;
	const session = providedSession || (await mongoose.startSession());
	const ownsSession = !providedSession;
	const useCache = options.cache !== undefined ? options.cache : options.useCache !== false;
	...
	try {
		...
		const header = await _createHeader(payload, performedBy, session);
		...
		const syncedHeader = await _syncHeaderData(header._id, session);
