...
const SEED_GROUP_ID = "seed-stock-2026-02";
...
const MOVEMENT_PLAN = [
  {
    ...
    movements: [
      ...
      { type: "SALE", quantity: 2, unitPrice: 5500000, buyerEmail: "buyer@test.com", paymentMethod: "CARD" },
    ],
  },
  {
    sku: "MODE-002",
    movements: [
      ...
      { type: "SALE", quantity: 1, unitPrice: 450000, buyerEmail: "buyer@test.com", paymentMethod: "MOBILE_MONEY" },
    ],
  },
  {
    sku: "BIO-001",
...
async function seedStockMovements() {
  ...
  await StockMovement.deleteMany({ groupId: SEED_GROUP_ID });
  ...
  for (const plan of MOVEMENT_PLAN) {
    ...
    const buyerByEmail = {};
    const getBuyer = async (email) => {
      if (!email) return null;
      if (buyerByEmail[email] !== undefined) return buyerByEmail[email];
      const buyer = await User.findOne({ email });
      buyerByEmail[email] = buyer;
      return buyer;
    };

    // Start from the larger of cached stock and calculated stock to avoid negatives
    const snapshot = await StockMovement.calculateStock(product._id);
    ...
    for (const move of plan.movements) {
      ...
      const buyer = await getBuyer(move.buyerEmail);
      ...
      const movement = new StockMovement(movementDoc);
      await movement.save();
      createdCount += 1;
    ...
    const updatedStock = await StockMovement.calculateStock(product._id);
